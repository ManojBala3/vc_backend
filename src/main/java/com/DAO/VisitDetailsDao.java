...
public interface VisitDetailsDao extends JpaRepository<VisitDetails, Integer>{
	...
	@Query(value="select count(*) from visit_details vd inner join customer_details cd on vd.cust_id=cd.cust_id where cd.mobile_no like ?1 and vd.queue_status='Waiting' and date(vd.visit_date)=current_date ",nativeQuery = true)
